...
paint_zona_puska::paint_zona_puska(QVector<QPointF> points, QObject *parent) : tochki(points), QGraphicsObject()
{
    calculate_polygon();
}


paint_zona_puska::~paint_zona_puska() {

}


QRectF paint_zona_puska::boundingRect() const {
...
QPainterPath paint_zona_puska::shape() const {
...
void paint_zona_puska::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    painter->setPen(Qt::red);
    uint8_t red = 176, green = 217, blue = 89, transparency = 70; // всё от 0 до 255
...
void paint_zona_puska::calculate_polygon()
{
    QVector<float> mass_X;
    QVector<float> mass_Y;
    for (int i = 0; i < tochki.size(); i++) {
    ...
    double sumX = 0,
           sumY = 0;
    for (int j = 0; j < tochki.size(); j++) {
        sumX += tochki[j].x();
        sumY += tochki[j].y();
    }
    central_point = {sumX/tochki.size(), sumY/tochki.size()};
}


void paint_zona_puska::set_end_point(QPointF end_point) {
    tochki.last() = end_point;
    calculate_polygon();
}


void paint_zona_puska::add_point(QPointF new_point) {
